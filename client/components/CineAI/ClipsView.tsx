...
export function ClipsView({ project, onProjectUpdate }: ClipsViewProps) {
  ...
  const cameraShots: CameraShot[] = [
    ...
    { id: "tilted", name: "Tilted", icon: <Monitor className="w-3 h-5" /> },
    {
      id: "handheld",
      name: "Handheld",
      icon: <Smartphone className="w-3 h-5" />,
    },
    {
      id: "steadicam",
      name: "Steadicam",
      icon: <Tablet className="w-3 h-5" />,
    },
  ...
  const mockScenes = [
    {
      ...
      imageUrl:
        "https://api.builder.io/api/v1/image/assets/TEMP/dfe9dab9ac4d667b29a86b0d6ef29dab1153e28c?width=116",
    },
    {
      id: "2",
      title: "Laboratory",
      imageUrl:
        "https://api.builder.io/api/v1/image/assets/TEMP/a75c8c807cc2b335f7498d9afa932903260741f5?width=116",
    },
    {
      id: "3",
      title: "Dark City",
      imageUrl:
        "https://api.builder.io/api/v1/image/assets/TEMP/6ec4fcbe7e6693b641dc0207f0840887d55642c4?width=116",
    },
  ];
  ...
  const mockCharacters = [
    {
      ...
      title: "Nova Chen",
      imageUrl:
        "https://api.builder.io/api/v1/image/assets/TEMP/5821b6aa396b0511f311c392689060b59165fad7?width=116",
    },
    {
      id: "2",
      title: "Victor Crane",
      imageUrl:
        "https://api.builder.io/api/v1/image/assets/TEMP/327c573275f90714e5fc082eb19efa30fb5af628?width=116",
    },
    {
      id: "3",
      title: "Dr. Elias Grey",
      imageUrl:
        "https://api.builder.io/api/v1/image/assets/TEMP/a4e4a4142936de335afe5db4326fa663156cf842?width=116",
    },
    {
      id: "4",
      title: "Maya Rodriguez",
      imageUrl:
        "https://api.builder.io/api/v1/image/assets/TEMP/bfab3bf8103c4269e3dc36705e7b3b4d55de5f86?width=116",
    },
    {
      id: "5",
      title: "Cipher",
      imageUrl:
        "https://api.builder.io/api/v1/image/assets/TEMP/9c9f023457482aacb444e53278bc0ce55e5370a5?width=116",
    },
  ];
  ...
  const handleSliderChange = (
    key: "temperatureValue" | "qualityValue" | "lengthValue",
    value: number,
  ) => {
    onProjectUpdate({
      ...project,
  ...
  const handleGenerateClip = () => {
    ...
    if (!project.scene || !project.character || !project.plotBeat) {
      alert(
        "Please select a scene, character, and plot beat before generating a clip.",
      );
      return;
    }
        ...
        <div className="flex gap-4 mb-6">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              ...
              >
                {project.scene && project.scene.imageUrl ? (
                  <img
                    src={project.scene.imageUrl}
                    alt={project.scene.title}
            ...
            <DropdownMenuContent
              align="start"
              side="bottom"
              className="w-48 bg-[hsl(var(--cine-card))] border-[hsl(var(--cine-border))]"
            >
              {mockScenes.map((scene) => (
                <DropdownMenuItem
                  key={scene.id}
                  onSelect={() => handleItemSelect(scene, "scene")}
                  className="text-white focus:bg-[hsl(var(--cine-sidebar))]"
                >
                  {scene.title}
                </DropdownMenuItem>
              ))}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              ...
              >
                {project.character && project.character.imageUrl ? (
                  <img
                    src={project.character.imageUrl}
                    alt={project.character.title}
                    className="w-full h-full object-cover rounded-md"
                  />
                ) : (
                  <>
            ...
            <DropdownMenuContent
              align="start"
              side="bottom"
              className="w-56 bg-[hsl(var(--cine-card))] border-[hsl(var(--cine-border))]"
            >
              {mockCharacters.map((character) => (
                <DropdownMenuItem
                  key={character.id}
                  onSelect={() => handleItemSelect(character, "character")}
                  className="text-white focus:bg-[hsl(var(--cine-sidebar))]"
                >
                  {character.title}
                </DropdownMenuItem>
          ...
          <DropdownMenu>
            ...
            >
              {mockPlotBeats.map((beat) => (
                <DropdownMenuItem
                  key={beat.id}
                  onSelect={() => handleItemSelect(beat, "plotBeat")}
                  className="text-white focus:bg-[hsl(var(--cine-sidebar))]"
                >
                  {beat.title}
                </DropdownMenuItem>
          ...
          <div className="flex gap-3">
            ...
            {activeTab === "plotBeats" &&
              mockPlotBeats.map((plotBeat) => (
                ...
                >
                  <span className="text-xs text-center text-[hsl(var(--cine-text-muted))]">
                    {plotBeat.title}
                  </span>
                </div>
              ))}
